...
exports.wrapParser = void 0;
const proxy_vir_1 = require("proxy-vir");
...
function wrapParser(originalParser, parserName) {
    /** Create a multi-target proxy of parsers so that we don't block other plugins. */
    const parserProxy = (0, proxy_vir_1.createWrappedMultiTargetProxy)({
    ...
    function multilineArraysPluginPreprocess(text, options) {
        ...
        pluginsWithRelevantParsers.forEach((plugin) => {
            ...
            if (currentParser) {
                if (plugin?.name?.includes('prettier-plugin-sort-json')) {
        ...
        pluginsWithPreprocessor.forEach((pluginWithPreprocessor) => {
            const nextText = pluginWithPreprocessor.parsers?.[parserName]?.preprocess?.(processedText, {
                ...options,
                plugins: pluginsFromOptions.filter((plugin) => plugin.pluginMarker !== plugin_marker_1.pluginMarker),
            });
